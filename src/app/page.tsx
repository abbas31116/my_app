...
export default function Home() {
  const item = (title: string, image: string, value: string) => (
    <div className=" flex items-start gap-2">
      <Image className="" src={image} alt={""} width={50} height={50} />
      <div className="space-y-2">
        <Title className="text-primary" title={title} />
        <Title
          className="text-primary text-right"
          title={convertToPersianNumber(value)}
        />
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div className=" grid grid-cols-2 w-fill h-100 m-15  place-content-center place-items-center">
        <div className="space-y-5 ">
          ...
          />
          <CustomText
            className="text-muted text-2xl w-170"
            title={
      ...
      <div className="place-items-center place-content-center ">
        <div className="place-items-center place-content-center space-y-5">
          ...
          <SubTitle
            className="text-popover-foreground text-4xl"
            title={"چرا ما بهترین انتخاب شما هستیم"}
          />
          <CustomText
            className="text-muted text-2xl"
            title={
              "که با همکاری نزدیک با شرکا و مشتریان ما توسعه یافته است، دانش صنعت، دهه ها تجربه، نبوغ و سازگاری را برای ارائه برتری به مشتریان خود ترکیب می کند."
            }
          />
        </div>
        ...
        <div className="bg-popover w-258 h-85 place-items-center place-content-center relative">
          ...
          <div className=" place-content-center h-full w-160 space-y-10 left-0  place-items-center absolute">
            ...
            <div className="grid grid-cols-2 place-items-start place-content-start gap-x-5 gap-y-10 ">
              {item("با ما تماس بگیرید", FRAME11, "09133333339")}
              {item("ایمیل", FRAME12, "info@dideh-ban.ir")}
              {item(
                "آدرس",
                FRAME12,
                "دفتر مرکزی: اصفهان، نجف آباد، اتاق بازرگانی",
              )}
            </div>
          </div>
        </div>
      ...
      <div className="place-items-center place-content-center m-40">
        <div className="bg-muted/30 w-250 h-90 place-items-center place-content-center rounded-2xl relative">
          <div className="bg-white w-65 rounded-b-[67px] place-content-center place-items-center p-5 absolute top-0">
            ...
            />
            <Title
              className="text-popover-foreground text-2xl mt-5"
              title={"افتخارات همکاری"}
            />
            <SubTitle
              className="text-muted text-2xl"
              title={"Partnership honor"}
            />
          </div>
